...
//!
//! ## 子命令说明
//! - **compile**: 读取输入目录中的 HTML/CSS/JS 源文件，调用 `compiler` 库生成 Rust 代码，
//!   并创建完整的 Cargo 项目（含 Cargo.toml 和 main.rs），输出到指定目录。
//! - **run**: 先执行 compile，然后自动调用 `cargo run` 编译并运行生成的 Rust 项目。
//!
//! ## 数据流
//! ```text
//! index.html + style.css + app.js  →  compiler 库  →  生成的 Rust 代码
//!                                                           ↓
...
/// CLI 支持的两个子命令。
enum CommandType {
    /// `cli compile` — 仅生成代码，不运行
    Compile,
    /// `cli run` — 生成代码后自动 cargo run
    Run,
}
...
fn main() {
    ...
    let args = match parse_args() {
        ...
        Err(e) => {
            eprintln!("error: {e}");
            eprintln!(
                "usage: cli compile <input-dir> [-o <output-dir>] [--name <name>] [--title <title>] [--width <w>] [--height <h>]"
            );
            eprintln!("       cli run <input-dir> [-o <output-dir>] [--name <name>] [--title <title>]");
...
/// `cli compile` 子命令的处理函数。
///
/// 执行步骤：
/// 1. 获取当前工作目录（用于计算 Cargo.toml 中的相对路径依赖）
/// 2. 检查输入目录中是否存在 HTML/CSS/JS 源文件
/// 3. 调用 `compiler::compile_with_options()` 生成 Rust 源代码
/// 4. 创建输出目录结构
...
fn cmd_compile(args: &Args) {
    ...
    let (css_refs, js_refs) = compiler::html::extract_references(&html_src);
    ...
    let rust_code = compiler::compile_with_options(
