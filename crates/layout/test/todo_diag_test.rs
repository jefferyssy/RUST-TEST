...
use style::cascade::compute_element_style_with_node;
...
fn test_todo_app_clear_all_text_position() {
    ...
    assert!(
        (clear_all_text.x + clear_all_text.width) <= btn_content_right + 3.0,
...
fn test_partial_layout_input_typing_with_li_children() {
    ...
    // 使用增量布局（仅 todo_input 被标记为脏）
    engine.layout(&mut root2, viewport);

    dump_positions(&root2, "after typing layout:");

    let footer_y_after = find_by_class(&root2, "footer").unwrap().rect.y;
    let todo_h_after = find_by_id(&root2, "todo-list").unwrap().rect.height;

    // ===== 断言 =====
    assert!(
        (todo_h_full - todo_h_after).abs() < 1.0,
        "todo_list height should NOT change: full={:.1} after={:.1}",
        todo_h_full, todo_h_after
    );

    assert!(
        (footer_y_full - footer_y_after).abs() < 1.0,
        "footer y should be STABLE: full={:.1} after={:.1}",
        footer_y_full, footer_y_after
    );

    eprintln!(
        "[diag partial] PASS: footer stable at y={:.1} (full={:.1})",
        footer_y_after, footer_y_full
    );
}
...
fn test_runtime_exact_partial_layout_body_root() {
    ...
    // ★ 关键：使用完整布局
    engine.layout(&mut body_root2, viewport);

    let footer_y_after = find_by_class(&body_root2, "footer").unwrap().rect.y;
    let todo_h_after = find_by_id(&body_root2, "todo-list").unwrap().rect.height;

    eprintln!(
        "[diag runtime test] AFTER layout (body root): footer.y={:.1} todo_list.h={:.1}",
        footer_y_after, todo_h_after
    );

    // ===== 断言 =====
    assert!(
        (todo_h_full - todo_h_after).abs() < 2.0,
        "todo_list height should NOT change: full={:.1} after={:.1}",
        todo_h_full, todo_h_after
    );
    ...
    assert!(
        (footer_y_full - footer_y_after).abs() < 1.0,
        "footer y should be STABLE (body root): full={:.1} after={:.1}",
        footer_y_full, footer_y_after
    );
